...
### 12 V buzzer — physical anchor alarm

An optional 12 V piezo buzzer wired to a relay on the Pi's GPIO will sound loud on-board when anchor drag or a Guardian zone breach is detected — waking crew even when phones are dead, silenced, or in another cabin.

#### Why sysfs, not a GPIO library?

The code shells through the kernel sysfs interface (`/sys/class/gpio/`) rather than an npm GPIO package. Sysfs gives persistent line ownership (the relay stays in its last state between writes) without spawning a child process on every toggle, works on Pi 4 and Pi 5 via the RP1 compatibility shim, and requires zero npm dependencies. The trade-off is that sysfs is officially deprecated in newer kernels — but it still works on Raspberry Pi OS Bookworm, and will be updated here if the kernel removes it. If GPIO initialisation fails (e.g. on a Mac during development), the code falls back silently to mock mode and logs every toggle transition to stdout, so all code paths are testable off the boat.

#### Wiring

```
Pi BCM 17 (pin 11)  →  relay module IN
Relay NO             →  buzzer +  (switched)
12 V +               →  relay COM
Buzzer −             →  12 V GND
```

Use any 3.3 V-compatible 5 V relay module (HiLetgo SRD-05VDC-SL-C or equivalent). The relay drives the 12 V circuit; the Pi GPIO only sees the low-voltage control signal. A continuous-tone piezo rated for 12 V at ~100 dB works best. The buzzer pattern is generated by software: anchor drag → 3 urgent 150 ms bursts + 700 ms pause; Guardian zone breach → slow 800 ms on/off. Both patterns loop until crew silence the buzzer via the anchor page.

**Default GPIO pin:** BCM 17 (physical pin 11). Change with `buzzer.gpioPin` in `anchor-api-config.json`.

#### Permissions for the systemd service user

The `pi` user (or whatever user runs `anchor-api.service`) must be in the `gpio` group:
...
If the group doesn't exist (rare on Bookworm):
...
For Pi 5 (RP1 chip uses `gpiochip4`), sysfs should still work via the compatibility layer. If it doesn't, replace the sysfs calls in `anchor-api.js` with `gpioset gpiochip4 17=1` / `gpioset gpiochip4 17=0` from `libgpiod-tools` (already installed on Bookworm).

#### Configuration (`anchor-api-config.json`)

```json
"buzzer": {
  "enabled": false,          // must be true to activate
  "gpioPin": 17,             // BCM pin number
  "rearmSeconds": 30,        // seconds until buzzer re-arms after Silence is pressed
  "rearmOnWorsening": true   // re-arm immediately if drag distance increases >5 m
}
```

Feature is completely OFF by default — existing installs without a buzzer are unaffected.

#### Alarm behaviour

| Condition | Pattern |
|---|---|
| Anchor drag or GPS loss | 3 × 150 ms bursts + 700 ms pause (loops) |
| Guardian zone breach | 800 ms on / 800 ms off (loops) |
| Both active | Drag pattern wins |
| Test (Settings page) | Drag pattern for 3 s then stops |

**Silencing:** a large SILENCE button appears on the anchor page while the buzzer is sounding. Press it to stop the sound immediately — visual alarm state (red banner, overlay) is unchanged. The buzzer re-arms after `rearmSeconds`. A countdown badge replaces the button while silenced. Silencing does not suppress Pushover notifications.

#### Deploy steps (after updating the repo on the Pi)

```bash
# 1. Pull the updated files
cd ~/oroboro-dashboard && git pull

# 2. Edit anchor-api-config.json on the Pi to enable the buzzer
nano anchor-api-config.json
# set "enabled": true, and set "gpioPin" to your wired BCM pin

# 3. Restart the service
sudo systemctl restart anchor-api

# 4. Verify GPIO initialised
sudo journalctl -u anchor-api -n 20
# look for: [buzzer] GPIO BCM17 ready (sysfs)
# or:       [buzzer] mock mode  (if GPIO access failed)

# 5. Test from Settings page → Alarms → Test button
#    You should hear the 3-burst pattern for 3 seconds.
```
